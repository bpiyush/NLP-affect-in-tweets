...
f = open(path + subfolders[0] + fol1[2] + 'NRC-Hashtag-Emotion-Lexicon-v0.2.txt')
rawf = f.read()

p1 = re.compile(r"[a-z]+\t[#a-z\d/]+\t\d.\d{1,5}")
m1 = p1.findall(rawf)
...
for x in m1:
    ...
    if t[0] in emotions:
        if t[1]=='#':
            y = t[1] + t[2]
            word_list.append([t[0], y, float(t[3])])
        else:
            if t[2] not in ['me', 'na', 'not', 'ta']:
                word_list.append([t[0], t[1], float(t[2])])
...
np.save('emotion_intensity_feature', feat_vecs)
