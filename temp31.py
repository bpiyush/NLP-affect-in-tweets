...
f = open(path + subfolders[0] + fol1[4] + 'HS-unigrams.txt')
rawf = f.read()
g = open(path + subfolders[0] + fol1[3] + 'HS-AFFLEX-NEGLEX-unigrams.txt')
rawg = g.read()
h = open(path + subfolders[0] + fol1[0] + 'Emoticon-AFFLEX-NEGLEX-unigrams.txt')
rawh = h.read()

total_file = rawf + '\n' + rawg + '\n' + rawh
p1 = re.compile(r"[?!~#a-z@\d_<>%*\\=/\^$&+_A-Z']+\t-\d[\.]{0,1}\d{0,3}\t")
m1 = p1.findall(total_file)
...
for x in m1:
    t = tokenize(x)
    #print(t)
    word_list.append([t[0], float(t[1])])


#print(word_list[0:300])
...
for i in tqdm.trange(4):
    ...
    for j,x in enumerate(a):
        #print("Sentence number: ", j + sum_arr[i]*bool(i!=0))
        y = 0
        h = preProcessTweet(x)
        for f in h:
            if f in words:
...
np.save('hashtag_intensity_feature', feat_vecs)
